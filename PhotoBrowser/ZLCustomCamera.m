...
- (void)setDelegate:(id<CameraToolViewDelegate>)delegate
{
    ...
    _delegateFlag.onEdit = [delegate respondsToSelector:@selector(onEditClick)];
...
- (void)layoutSubviews
{
    ...
    self.editBtn.layer.cornerRadius = height*kBottomViewScale/2;
...
- (void)setupUI
{
    ...
    self.editBtn.backgroundColor = [kRGB(244, 244, 244) colorWithAlphaComponent:.9];
    [self.editBtn setImage:GetImageWithName(@"zl_camera_edit") forState:UIControlStateNormal];
    [self.editBtn addTarget:self action:@selector(onEditAction) forControlEvents:UIControlEventTouchUpInside];
...
//    NSLog(@"%ld--%ld",self.allowEditImage,_allowEditImage);
    ...
    CGRect editRect = self.editBtn.frame;
    editRect.origin.x = GetViewWidth(self)/2 - editRect.size.width/2;
...
-(void)onEditAction{
    if (_delegateFlag.okClick) [self.delegate performSelector:@selector(onEditClick)];
}
...
-(void)onEditClick{
    ...
    //    weakSelf.takedImage
//    __weak typeof(self) weakSelf = self;
    UIImage *img = self.takedImage;
    //    [UIImage imageNamed:@"zl_videoView.png"];
//    PHAssetCreationRequest *assetCreationRequest = [PHAssetCreationRequest creationRequestForAssetFromImage:img];
    [ZLPhotoManager saveImageToAblum:img completion:^(BOOL suc, PHAsset *asset) {
        NSLog(@"%@",asset);
